...
from agentic_workflow import DEFAULT_GITHUB_REPO, run_workflow
...
def render_plan(state_payload: dict) -> None:
    rows = [
        ...
        for step in state_payload["plan"]
    ]
...
def main() -> None:
    st.set_page_config(page_title="SuppBro HW6", layout="wide")
    init_state()

    st.title("SuppBro HW6 agentic workflow")
    st.caption("Chat + trace dashboard for HW4 retrieval and HW5 external tools.")

    with st.sidebar:
        st.header("Run settings")
        repo = st.text_input("GitHub repo", DEFAULT_GITHUB_REPO)
        issue_number_text = st.text_input("Issue number override", "")
        allow_external = st.checkbox("Allow Stack Overflow/community search", value=False)
    ...
    if question:
        ...
        with st.spinner("Running controlled workflow..."):
            state = run_workflow(
                question,
                repo=repo,
    ...
    if not state_payload:
        st.info("Run a chat question to see route, plan, observations, and current state.")
        return
